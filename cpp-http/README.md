...
./setup.sh
make distclean
wget -O /tmp/defconfig https://github.com/unikraft/catalog-core/tree/scripts/cpp-http/scripts/defconfig/qemu.x86_64
...
This will configure, build and run CPP HTTP on Unikraft.
...
./setup.sh
make distclean
wget -O /tmp/defconfig https://github.com/unikraft/catalog-core/tree/scripts/cpp-http/scripts/defconfig/qemu.arm64
UK_DEFCONFIG=/tmp/defconfig make defconfig
make -j $(nproc)
sudo ip link set dev virbr0 down
sudo ip link del dev virbr0
...
Similar to the `x86_64` build, this will configure, build and run CPP HTTP on Unikraft.
