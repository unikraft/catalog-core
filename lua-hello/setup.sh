...
if ! test -d workdir/libs/lua; then
  if test -d ../repos/libs/lua; then
    ln -sfn ../../../repos/libs/lua workdir/libs/lua
  else
    git clone https://github.com/unikraft/lib-lua workdir/libs/lua
  fi
fi

if ! test -d workdir/libs/musl; then
  if test -d ../repos/libs/musl; then
