# Flask 3.0 Python3.10 on Unikraft

Build and run a Flask 3.0 program on Unikraft.
Follow the instructions below to set up, configure, build and run Flask 3.0 Python3.10.
...
./setup.sh
make distclean
wget -O /tmp/defconfig https://raw.githubusercontent.com/unikraft/catalog-core/refs/heads/scripts/flask3.0-python3.10/scripts/defconfig/qemu.x86_64
...
qemu-system-x86_64 \
  ...
  -kernel workdir/build/flask3.0-python3.10_qemu-x86_64 \
  -initrd ./initrd.cpio \
  -append 'rootfstype=ramfs console=ttyS0 \
    vfs.fstab=["initrd0:/:extract::ramfs=1"] \
    env.vars=[PYTHONHOME="/usr/local/lib/python3.10" PYTHONPATH="/usr/local/lib/python3.10:/usr/local/lib/python3.10/site-packages"] \
    -- /app/server.py'
```

This will configure, build and run Flask 3.0 Python3.10 on Unikraft.
...
./setup.sh
make distclean
wget -O /tmp/defconfig https://raw.githubusercontent.com/unikraft/catalog-core/refs/heads/scripts/flask3.0-python3.10/scripts/defconfig/qemu.arm64
UK_DEFCONFIG=/tmp/defconfig make defconfig
make -j $(nproc)
test -d ./rootfs/ || docker build -o ./rootfs -f Dockerfile .
test -f initrd.cpio || ./workdir/unikraft/support/scripts/mkcpio initrd.cpio ./rootfs/
qemu-system-aarch64 \
  ...
  -kernel workdir/build/flask3.0-python3.10_qemu-arm64 \
  -initrd ./initrd.cpio \
...
Similar to the `x86_64` build, this will configure, build and run Flask 3.0 Python3.10 on Unikraft.
...
The unikernel application image file is `workdir/build/flask3.0-python3.10_<plat>-<arch>`, where `<plat>` is the platform name (`qemu`, `fc`, `xen`), and `<arch>` is the architecture (`x86_64` or `arm64`).
...
1. The Python 3.10 standard library from the upstream Python container image.
1. Flask and its dependencies installed from [`requirements.txt`](requirements.txt).
1. The application entry point [`server.py`](server.py).
...
```console
$ curl localhost:8080
Hello from Flask on Unikraft!
```

This means that Flask 3.0 Python3.10 runs on Unikraft.
...
```console
sudo xl destroy flask3.0-python3.10
```
...
The current program is [`server.py`](server.py), a simple Flask application.
...
1. If the new application depends on additional Python packages, update [`requirements.txt`](requirements.txt).

1. Update the command line arguments for starting the virtual machine, to use the new command line arguments instead of `-- /app/server.py`.
...
Keep the root filesystem aligned with Python `3.10.x`, because the Unikraft image is built against `lib-python3` for Python 3.10.

To switch to a different Python `3.10.x` base image, do the following:

1. Update the [`Dockerfile`](Dockerfile) to use the desired `python:3.10.x` image tag.

1. Keep the `COPY --from=base` line in the [`Dockerfile`](Dockerfile) pointed at `python3.10`.
...
Below find instructions on running Flask 3.0 Python3.10 on QEMU with 9pfs support.

You need to use these contents for the [`Config.uk`](Config.uk) configuration file:

```text
config APPFLASK3_0_PYTHON3_10
bool "Configure Flask 3.0 Python3.10 application with 9pfs as rootfs"
default y
        select LIBPYTHON3
        select LIBPYTHON3_MAIN_FUNCTION
        select LIBZLIB
        select LIBPYTHON3_EXTENSION_ZLIB
        select LIBVFSCORE
...
This means you replace these four lines in [`Config.uk`](Config.uk):
...
Finally, after configuring networking, use the commands below to run Flask 3.0 Python3.10 on QEMU/x86_64 with 9pfs support:
...
cp -r ./rootfs ./9pfs-rootfs
 ...
 qemu-system-x86_64 \
  ...
  -kernel workdir/build/flask3.0-python3.10_qemu-x86_64 \
  -append 'flask3.0-python3.10_qemu-x86_64 vfs.fstab=["fs0:/:9pfs:::"] env.vars=[PYTHONPATH="/usr/local/lib/python3.10:/usr/local/lib/python3.10/site-packages"] -- /app/server.py'
```
